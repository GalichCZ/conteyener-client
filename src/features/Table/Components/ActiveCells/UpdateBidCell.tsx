...
const UpdateBidCell: FC<Props> = ({ bid, hidden }) => {
  ...
  if (user && user.role !== roleType1[user.role as keyof typeof roleType1]) return <></>

  return (
    <>
      {open &&
        createPortal(
          <UpdateBidModal followBid={bid} open={open} setOpen={setOpen} />,
          document.body
        )}
      <TableCell.Cell onClick={handleOpen} className={`cursor-pointer`}>
        {formatDate(bid.request_date)}
      </TableCell.Cell>
    </>
  )
}
