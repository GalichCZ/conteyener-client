...
import StoreInfoModal from '@/features/StoreInfoModal/components/StoreInfoModal.tsx'
...
const StoreInfoCell: FC<Props> = ({ bid }) => {
  ...
  if (user && user.role !== roleType4[user.role as keyof typeof roleType4]) return <></>
  ...
  return (
    <>
      {open &&
        createPortal(
          <StoreInfoModal open={open} setOpen={setOpen} storeId={bid.store._id} />,
          document.body
        )}
