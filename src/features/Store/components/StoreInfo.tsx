...
const StoreInfo: FC<Props> = ({ store }) => {
  ...
  return (
    <>
      {openDelete &&
        createPortal(
          <StoreDeleteModal open={openDelete} setOpen={setOpenDelete} storeId={store._id} />,
          document.body
        )}
      {open &&
        createPortal(
          <StoreUpdateModal setOpen={setOpen} store={store} open={open} />,
          document.body
        )}
      <RowWrap className="mt-1">
        ...
        <PoleWrap>
          <Button
            ...
            className="border-red-500 border-2 text-red-500"
            text="Удалить"
          />
        </PoleWrap>
      </RowWrap>
