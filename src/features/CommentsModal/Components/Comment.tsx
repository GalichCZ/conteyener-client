...
const Comment: FC<Props> = ({ comment }) => {
    ...
    return (
        <div className="flex relative mt-10 gap-4">
            <span
                className="absolute -top-1 left-0 -translate-y-full">{comment.comment_creator?.first_name} {comment.comment_creator?.last_name}</span>

            <Input.TextArea defaultValue={comment.comment_text} onChange={areaHandle}/>

            <span className="absolute right-0 bottom-0 translate-y-1">{formatDate(comment.comment_date)}</span>
